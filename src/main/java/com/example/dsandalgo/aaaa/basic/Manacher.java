...
public class Manacher {
    ...
    public static int manacher(String str) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            while ((i + pArray[i]) < n && (i - pArray[i]) > -1) {
                if (chars[i + pArray[i]] == chars[i - pArray[i]]) {
                    r++;
                    c = i;
                    pArray[i]++;
                } else {
    ...
    /**
     * 将一个字符串转化成一个manacher字符串
     *
     * @param str
     * @return manacharString
     */
    public static char[] manacharString(String str) {
        ...
        char[] targetCharArray = new char[n * 2 + 1];
        int index = 0;
        for (int i = 0; i < targetCharArray.length; i++) {
            targetCharArray[i] = (i & 1) == 0 ? '#' : originCharArray[index++];
        }
